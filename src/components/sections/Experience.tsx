      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        ...
        <div ref={ref} className="fade-up space-y-6">
          {EXPERIENCES.map((exp, i) => (
            <div
              key={exp.id}
              className="terminal-window p-6 hover:border-terminal-green/30 transition-all duration-300 glow-border"
            >
              {/* Header */}
              ...
              <div className="flex flex-wrap gap-2 pt-3 border-t border-terminal-border">
                {exp.tech.map((t) => (
                  <span
                    key={t}
                    className="px-2 py-0.5 text-[10px] border border-terminal-border text-terminal-muted"
                  >
                    {t}
                  </span>
                ))}
