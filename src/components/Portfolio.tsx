/* ============================================================
   Portfolio.tsx — Sección de proyectos destacados

   Cada proyecto se muestra como una tarjeta estilo terminal con:
     - Índice + título + badge de estado
     - Descripción breve
     - Lista de highlights (logros clave)
     - Tags de tecnologías usadas
     - Links a repo y/o demo (si existen)
   ============================================================ */
"use client";
...
/* Estilos del badge según el estado del proyecto */
  ...
  return (
    <section id="portfolio" className="py-24 bg-terminal-surface/20 border-y border-terminal-border">
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        ...
        {/* Grid de 2 columnas en desktop — cada tarjeta es un proyecto */}
        <div ref={ref} className="fade-up grid md:grid-cols-2 gap-6">
          {PROJECTS.map((proj, i) => (

            <div key={proj.id} onClick={() => router.push(`/projects/${proj.slug}`)}
              
                /* "group" permite usar group-hover en los hijos para
                   efectos coordinados al pasar el cursor por la tarjeta */
                className="terminal-window p-5 flex flex-col gap-4 hover:border-terminal-green/40 transition-all duration-300 group cursor-pointer"
              >
                {/* ── Encabezado: índice + título + badge ────────── */}
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-center gap-2">
                    <span className="text-terminal-muted text-xs shrink-0">
                      [{String(i + 1).padStart(2, "0")}]
                    </span>
                    {/* Título se ilumina con glow al hacer hover en la tarjeta */}
                    <h3 className="text-terminal-green text-sm font-bold group-hover:glow-green transition-all">
                      {proj.title}
                    </h3>
                  </div>

                  {/* Badge de estado: ACTIVE / IN_PROGRESS / ARCHIVED */}
                  <span className={`text-[10px] px-2 py-0.5 border shrink-0 ${STATUS_STYLE[proj.status]}`}>
                ...
                {/* ── Highlights: logros clave del proyecto ─────────
                    El checkmark "✓" en verde da sensación de "completado" */}
                ...
                {/* ── Links: repo y/o demo (solo si existen) ────────
                    stopPropagation evita que el click active el Link padre */}
